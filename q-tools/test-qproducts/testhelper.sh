...
#set -x
    ...
    node substVars "templates/apiproduct-template.json" "proxyName" "${apiProxyName}" "productName" "${apiProductName}" >  "${apiProductName}".tmp
    node substVars "${apiProductName}".tmp "quotaAmount" "${setQuota}" "quotaInterval" "${setInterval}"  "quotaTimeUnit" "${setTimeUnit}" >  "${apiProductName}".json
    rm "${apiProductName}".tmp
...
function setupUserData() {
    ...
    qweight="$8"
    # ---- ---- ---- ---- ----
    node substVars "templates/usrdata.json" "org" "${MOCHA_ORG}" "env" "${MOCHA_ENV}" "devguy" "${developerName}" "devApp" "${apiProductName}" >  usrdata.tmp
    node substVars usrdata.tmp "pass" "${MOCHA_PASSWORD}" "user" "${MOCHA_USER}" >  usrdata2.tmp
    rm usrdata.tmp
    node substVars usrdata2.tmp "quotaAmount" "${appQuota}" "quotaInterval" "${appInterval}" "quotaTimeUnit" "${appTimeUnit}" "quotaWeight" ${qweight} >  usrdata3.tmp
    rm usrdata2.tmp
    #
    cp usrdata3.tmp ../usrdata.json

    pushd ..
    mykeys=$(node fetchkey.js)
    popd
    set -- $mykeys
    node substVars usrdata3.tmp "key" "$1" "secret" "$2" >  ../usrdata.json
